...
.hidden{display:none!important}.login-step{background:var(--card);border:1px solid var(--border);border-radius:18px;padding:24px;box-shadow:var(--shadow)}
.step-title{text-align:center;margin-bottom:18px}
.role-choice-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin:18px 0}
.role-choice{border:2px solid var(--border);background:#fff;border-radius:14px;padding:18px;cursor:pointer;text-align:left;transition:.15s;width:100%}
.role-choice:hover{border-color:var(--primary)}
.role-choice.selected{border-color:var(--primary);background:#eef6ff}
.selected-role{margin:0 0 14px;padding:10px 12px;border-radius:10px;background:#eef6ff;color:var(--primary2);font-weight:700;text-align:center}
.next-wrap,.submit-wrap{display:flex;justify-content:center;margin-top:18px}
.next-wrap .btn,.submit-wrap .btn{min-width:150px}
.captcha-box{margin-top:14px;padding:14px;border:1px solid var(--border);border-radius:12px;background:#f8fafc}
.captcha-question{font-weight:800;font-size:17px;margin-bottom:8px;text-align:center}
.back-btn{border:0;background:transparent;color:var(--primary);font-weight:700;cursor:pointer;margin-top:12px;width:100%}
@media(max-width:600px){.role-choice-grid{grid-template-columns:1fr}}
</style></head><body><div class="top"><div class="brand">Logistics Portal</div></div><div class="wrap login-wrap">
<div id="roleStep" class="login-step">
  <div class="step-title"><span class="role-badge">Step 1 of 2</span><h1>Select your role</h1><p class="muted">Choose whether you are the Owner or Client.</p></div>
  {message}
  <div class="role-choice-grid">
    <button type="button" id="clientChoice" class="role-choice" onclick="selectRole('client')"><div class="role-icon">👤</div><h3>Client</h3><p>Track shipments and submit new shipment inquiries.</p></button>
    <button type="button" id="ownerChoice" class="role-choice" onclick="selectRole('owner')"><div class="role-icon">🧑‍💼</div><h3>Owner</h3><p>Manage inquiries, shipments, client updates and the executive dashboard.</p></button>
  </div>
  <div class="next-wrap"><button type="button" class="btn primary" onclick="goToCredentials()">Next</button></div>
</div>

<div id="credentialsStep" class="login-step hidden">
  <div class="step-title"><span class="role-badge">Step 2 of 2</span><h1>Sign in</h1><p class="muted">Enter the credentials for the selected role.</p></div>
  ...
  <form method="post" action="/login" class="login-form">
    ...
    <div class="captcha-box"><div class="captcha-question">CAPTCHA: {captcha_question}</div><label>Enter the answer</label><input name="captcha_answer" inputmode="numeric" autocomplete="off" required></div>
    <div class="submit-wrap"><button class="btn primary" type="submit">Sign In</button></div>
  </form>
  <button type="button" class="back-btn" onclick="goBackToRole()">← Change role</button>
</div>
</div>
<script>
function selectRole(role){
  document.getElementById('role').value=role;
  document.getElementById('ownerChoice').classList.toggle('selected',role==='owner');
  document.getElementById('clientChoice').classList.toggle('selected',role==='client');
  document.getElementById('selectedRole').textContent=role==='owner'?'Owner selected':'Client selected';
}
function goToCredentials(){
  const role=document.getElementById('role').value;
  if(!role){ alert('Please select Owner or Client first.'); return; }
  document.getElementById('roleStep').classList.add('hidden');
  document.getElementById('credentialsStep').classList.remove('hidden');
  document.querySelector('#credentialsStep input[name="username"]').focus();
}
